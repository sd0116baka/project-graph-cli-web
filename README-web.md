...
启动脚本会写入本机后端发现注册表，供 CLI 和桌面端发现当前后端：

```text
%TEMP%\project-graph-backends.json
```

如果同一个数据目录已经有健康的后端在运行，再次运行 `start-web.cmd` 会复用已有后端并刷新注册表，不会再启动一个重复实例。
...
桌面版欢迎页会显示同一套服务器项目面板。它会优先读取本机后端发现注册表，自动连接已经运行的本机后端；也可以手动输入 `http://127.0.0.1:<端口>` 或局域网地址。

桌面版里的“启动本机后端”按钮会寻找 `scripts/start-web.ps1`，并以 local-only 模式启动，只绑定 `127.0.0.1`。如果分发包把启动脚本放在其他位置，可以设置：

```powershell
$env:PROJECT_GRAPH_BACKEND_START_SCRIPT = "C:\path\to\start-web.ps1"
```

打开 `server:` 项目后，桌面端读写、锁、历史恢复和删除都通过后端 API 完成；直接打开本地 `.prg` 文件仍然保留原来的本地文件流程。
